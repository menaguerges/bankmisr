...
namespace BankMisr
{
    ...
    public class TestHelper : ITestHelper
    {
        ...
        {
            if (!GetCurrentMethod(level: 2).Equals("WaitFor"))
        ...
        {
            if (!GetCurrentMethod(2).Equals("WaitFor"))
        ...
        {
            if (!GetCurrentMethod(level: 2).Equals("WaitForPageReadyState") && !GetCurrentMethod(level: 2).Equals("ClickOn") && !GetCurrentMethod(level: 2).Equals("ScrollTo") && !GetCurrentMethod(level: 2).Equals("OpenNewTab"))
        ...
        public virtual void FrontendLogin(string username, string password, By[] loginLocators)
        {
            ...
            try
            {
                ...
                SendKeys(loginLocators[index], username);
                index++;
                SendKeys(loginLocators[index], password);
                index++;

                ClickOn(FindElement(loginLocators[index]), false);
        ...
        public virtual void WaitTillPageLoad(By loaderLocator, int numberOfWaits = 2)
        {
            try
            {
                ...
                {
                    WaitFor(loaderLocator, Condition.Invisible);
                    Thread.Sleep(2000);
                    counter++;
                }
            }
        ...
        public virtual void OpenRequestFromRequestsList(string requestId, int statusIndexinGrid, By requestTextBoxLocator, By searchButtonLocator, string expectedStatus)
        {
            try
            {
                do
                {
                    ...
                    Thread.Sleep(1000);
                } while (!IsElementPresent(By.LinkText(requestId)));

                WaitFor(By.LinkText(requestId), Condition.Visible);

                CheckStatusInGrid(requestId, statusIndexinGrid, expectedStatus);

                ClickOn(By.LinkText(requestId), false);
            }
            catch (Exception e)
            {
                LogIssue("", "Exception occurred in function \"" + GetCurrentMethod() + "\". Please check attached screenshot: " + TakeScreenshot(), false, true);
        ...
        public virtual void TakeAction(string actionValue, By actionControlLocator, By commentTextboxLocator, By saveButtonLocator)
        {
            try
            {
                WaitFor(actionControlLocator, Condition.Visible);
                try
                {
                    SendKeys(commentTextboxLocator, "Action: " + actionValue);
                }
                catch (Exception)
                {
                    // ignored
                ...
                SelectByValue(actionControlLocator, actionValue);

                ClickOn(saveButtonLocator, false);
            }
            catch (Exception e)
            {
                LogIssue("", "Exception occurred in function \"" + GetCurrentMethod() + "\". Please check attached screenshot: " + TakeScreenshot(), false, true);
        ...
        public virtual void SearchRequest(string requestId, By requestTextBoxLocator, By searchButtonLocator)
        {
            try
            {
                WaitFor(requestTextBoxLocator, Condition.Visible);
                Clear(requestTextBoxLocator);
                SendKeys(requestTextBoxLocator, requestId);
                WaitFor(searchButtonLocator, Condition.Visible);
                SendKeys(searchButtonLocator, Keys.Enter);
            }
            catch (Exception e)
            {
                LogIssue("", "Exception occurred in function \"" + GetCurrentMethod() + "\". Please check attached screenshot: " + TakeScreenshot(), false, true);
        ...
        public virtual bool CheckStatusInGrid(string requestId, int statusIndexinGrid, string expectedRequestStatus)
        {
            try
            {
                WaitFor(By.PartialLinkText(requestId), Condition.Visible);
                ScrollTo(By.PartialLinkText(requestId));
                var reqRow = FindCertainParentOf(By.PartialLinkText(requestId));
                var tdElements = reqRow.FindElements(By.TagName("td"));

                var tableCellofStatus = tdElements.ElementAt(statusIndexinGrid);
                var statusIsCorrect = GetTextOf(tableCellofStatus).ToLower().Trim().Equals(expectedRequestStatus.ToLower().Trim());
                LogInfo(requestId, (statusIsCorrect ? "Correct Status in Grid: " : "Wrong Status in Grid: Expected Status is \"" + expectedRequestStatus + "\" and ") + "Visible Status is \"" + GetTextOf(tableCellofStatus) + "\"", false, false);
        ...
        public virtual void SetDate(DateTime date, By calenderButton, By calendarPopup, By previousMonth, By nextMonth, string dateFieldDescription = "")
        {
            try
            {
                ...
                LogCommands("", GetCurrentMethod() + " to " + (string.IsNullOrEmpty(dateFieldDescription) ? ("Element with Locator: " + calenderButton.ToString()) : dateFieldDescription), false, true);
            }
            catch (Exception e)
            {
                LogIssue("", "Exception occurred in function \"" + GetCurrentMethod() + "\". Please check attached screenshot: " + TakeScreenshot(), false, true);
        ...
        public virtual bool IsDriverOpen()
        {
            ...
            try
            {
                ...
                if (!GetCurrentMethod(level: 2).Equals("GetBrowserErrors") && !GetCurrentMethod(level: 2).Equals("CloseDriver"))
                    LogCommands("", GetCurrentMethod(), false, true);
                return true;
            }
            catch (Exception)
        ...
        {
            if (!GetCurrentMethod(level: 2).Equals("WaitForPageReadyState") && !GetCurrentMethod(level: 2).Equals("SwitchToFrame") && !GetCurrentMethod(level: 2).Equals("SwitchToFrameOfElement"))
                LogCommands("", GetCurrentMethod(), false, true);
        ...
        public virtual void SwitchToFrame(string frameId)
        {
            ...
            {
                SwitchToDefault();
                WaitFor(By.Id(frameId), Condition.FrameAvailabilityAndSwitchToIt);
            }
            catch (Exception e)
            {
                LogIssue("", "Exception occurred in function \"" + GetCurrentMethod() + "\". Please check attached screenshot: " + TakeScreenshot(), false, true);
        ...
        public virtual string TakeScreenshot(string screenshotName = "")
        {
            ...
            var previousMethod = GetCurrentMethod(2);
            string fileName;
        ...
        /// <summary>
        /// Send Mail: Sends Mail through SendGrid Api while attaching log files and screenshots if any were taken
        /// </summary>
        ...
        /// <summary>
        /// List of File Types that can be 
        /// ed. Used in UploadAttachments Function
        /// </summary>
